...
const fullDayEventVariants = cva("font-medium p-1 text-xs rounded-sm w-full", {
  variants: {
    variant: {
...
const CalendarDayView = () => {
  ...
  const timedEvents = events.filter((event) => event.start && !event.fullDate)

  return (
    <div className="flex flex-col h-full overflow-auto">
      {/* Full day events section */}
      {fullDayEvents.length > 0 && (
        <div className="border-b p-2 bg-muted/5 sticky top-0">
          <div className="flex flex-col gap-1">
            {fullDayEvents.map((event) => (
              <div
                key={event.id}
                className={cn(
                  fullDayEventVariants({ variant: event.color }),
                  "w-full",
                )}
              >
                {event.title}
      ...
      <div className="flex flex-1 relative pt-2">
        ...
        <div className="flex-1">
          {hours.map((hour) => (
            <div key={hour.toString()} className="h-20 border-t last:border-b">
              {timedEvents
                .filter((event) => event.start && isSameHour(event.start, hour))
                .map((event) => {
                  const hoursDifference = event.end
...
const CalendarWeekView = () => {
  ...
  return (
    <div className="flex flex-col relative overflow-auto h-full">
      ...
      {/* Full day events section */}
      <div className="flex border-b sticky top-10 bg-background/80 backdrop-blur-md z-10">
          ...
          {weekDates.map(({ date }, i) => {
            ...
            return (
              <div
                key={date.toString()}
                className={cn(
                  "p-1 border-l first:border-l-0",
                  [0, 6].includes(i) && "bg-muted/5",
                )}
              >
                {fullDayEvents.map((event) => (
                  <div
      ...
      <div className="flex flex-1">
        ...
        <div className="grid grid-cols-7 flex-1">
          {weekDates.map(({ date, hours }, i) => {
            const timedEvents = events.filter(
              (event) =>
                event.start && !event.fullDate && isSameDay(event.start, date),
            )
            ...
            return (
              ...
              >
                {hours.map((hour) => (
                  <div
                    key={hour.toString()}
                    className="h-20 border-t last:border-b"
                  >
                    {timedEvents
...
const CalendarMonthView = () => {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="grid grid-cols-7 gap-px sticky top-0 bg-background/80 backdrop-blur-md border-b">
        {weekDays.map((day, i) => (
          <div
            key={day}
            className={cn(
      ...
      <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
        {monthDates.map((_date) => {
          const currentEvents = events.filter((event) =>
            event.fullDate
              ? isSameDay(event.fullDate, _date)
              : event.start && isSameDay(event.start, _date),
          )
          ...
          return (
            ...
            >
              ...
              {currentEvents.map((event) => {
                const isFullDay = Boolean(event.fullDate)
